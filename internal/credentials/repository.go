...
type Repository interface {
	Upsert(ctx context.Context, cred *model.PlatformCredential) error
	GetByUserAndPlatform(ctx context.Context, userID, platform string) (*model.PlatformCredential, error)
	ListByUser(ctx context.Context, userID string) ([]model.PlatformCredential, error)
...
func (r *mysqlRepository) Upsert(ctx context.Context, cred *model.PlatformCredential) error {
	now := time.Now()
	cred.UpdatedAt = now
...
func (r *mysqlRepository) GetByUserAndPlatform(ctx context.Context, userID, platform string) (*model.PlatformCredential, error) {
	const q = `
...
func (r *mysqlRepository) ListByUser(ctx context.Context, userID string) ([]model.PlatformCredential, error) {
	const q = `
		SELECT id, user_id, platform, access_token_enc, refresh_token_enc,
		       token_expiry, scopes, platform_user_id, metadata, created_at, updated_at
		FROM platform_credentials
