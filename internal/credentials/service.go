...
type Service interface {
	Connect(ctx context.Context, userID string, req model.ConnectPlatformRequest) (*model.PlatformCredentialResponse, error)
	List(ctx context.Context, userID string) (*model.ListCredentialsResponse, error)
	Get(ctx context.Context, userID, platform string) (*model.PlatformCredentialResponse, error)
	Update(ctx context.Context, userID, platform string, req model.UpdatePlatformRequest) (*model.PlatformCredentialResponse, error)
...
func (s *service) Connect(ctx context.Context, userID string, req model.ConnectPlatformRequest) (*model.PlatformCredentialResponse, error) {
...
func (s *service) List(ctx context.Context, userID string) (*model.ListCredentialsResponse, error) {
	creds, err := s.repo.ListByUser(ctx, userID)
	if err != nil {
		return nil, err
	}
	out := make([]model.PlatformCredentialResponse, 0, len(creds))
	for i := range creds {
		out = append(out, *credentialToResponse(&creds[i]))
	}
	return &model.ListCredentialsResponse{Credentials: out}, nil
}

func (s *service) Get(ctx context.Context, userID, platform string) (*model.PlatformCredentialResponse, error) {
...
func (s *service) Update(ctx context.Context, userID, platform string, req model.UpdatePlatformRequest) (*model.PlatformCredentialResponse, error) {
...
func credentialToResponse(c *model.PlatformCredential) *model.PlatformCredentialResponse {
	var meta map[string]string
	if len(c.Metadata) > 0 {
		meta = cloneMeta(c.Metadata)
	}
	return &model.PlatformCredentialResponse{
		Platform:       c.Platform,
		Connected:      true,
