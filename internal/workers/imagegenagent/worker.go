...
const (
	tracerName     = "internal/workers/imagegenagent"
...
func (w *Worker) handle(ctx context.Context, msg *kafka.Message) error {
	...
	var event types.RefinedPromptEvent
...
func buildWorkflow(prompt string, style types.StyleParams, imageURL string) *types.ComfyWorkflowInput {
	clientID := uuid.New().String()

	nodes := map[string]types.ComfyNode{
		"1": {
			ClassType: "LoadImage",
	...
	return &types.ComfyWorkflowInput{
		Prompt:   nodes,
		ClientID: clientID,
	}
}
